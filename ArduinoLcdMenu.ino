...
void re_update() {
  ...
  if (re_counter > 999 || re_counter < -999) {
    re_counter = 0;
  }
...
void menu1_1() {
  ...
  if (re_cnt_prev != re_counter && refreshed == true) {
    ...
    if (startline < 0) {
      startline = 0; //startline min is 0
    }
    if (startline > 6) {
      startline = 6; //startline max is 6
    }
  }
...
void menu1_3() {
  ...
  if (m_1_3_editrow == 1 && re_cnt_prev != re_counter) {
    ...
    if (m_1_3_valNo < 0) {
      m_1_3_valNo = 9; //loop number back to 9 if under 0
    }
    if (m_1_3_valNo > 9) {
      m_1_3_valNo = 0; //loop number back to 0 if over 9
    }
  }
  ...
  if (m_1_3_editrow == 2 && re_cnt_prev != re_counter) {
    ...
    if (m_1_3_Value < -100) {
      m_1_3_Value = -50; //loop back to -50 if less than -100
    }
    if (m_1_3_Value > 100) {
      m_1_3_Value = 50; //loop back to 50 if over 100
    }
  }
...
void CheckChanges() {
  ...
  if (edit_mm && re_cnt_prev != re_counter) {
    ...
    if (mmi < 1) {
      mmi = 12; //loop number back to 12 if under 1
    }
    if (mmi > 12) {
      mmi = 1; //loop number back to 1 if over 12
    }
  }
  ...
  if (edit_dd && re_cnt_prev != re_counter) {
    ...
    if (ddi < 1) {
      ddi = 31; //loop number back to 31 if under 1
    }
    if (ddi > 31) {
      ddi = 1; //loop number back to 1 if over 31
    }
  }
  ...
  if (edit_yy && re_cnt_prev != re_counter) {
    ...
    if (yyi < 2022) {
      yyi = 2122; //loop number back to 2122 if under 2022
    }
    if (yyi > 2122) {
      yyi = 2022; //loop number back to 2022 if over 2122
    }
  }
  ...
  if (edit_hh && re_cnt_prev != re_counter) {
    ...
    if (hhi < 0) {
      hhi = 24; //loop number back to 24 if under 0
    }
    if (hhi > 24) {
      hhi = 0; //loop number back to 0 if over 24
    }
  }
  ...
  if (edit_mi && re_cnt_prev != re_counter) {
    ...
    if (mii < 0) {
      mii = 59; //loop number back to 59 if under 0
    }
    if (mii > 59) {
      mii = 0; //loop number back to 0 if over 59
    }
  }
  ...
  if (edit_ss && re_cnt_prev != re_counter) {
    ...
    if (ssi < 0) {
      ssi = 59; //loop number back to 59 if under 0
    }
    if (ssi > 59) {
      ssi = 0; //loop number back to 0 if over 59
    }
  }
...
void powersave() {
  ...
  if (button_pressed || re_cnt_prev != re_counter) {
    pwr_cycles = 0;
  }
